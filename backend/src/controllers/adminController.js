...
const createProduct = async (req, res) => {
  const {
    name, description, price, compare_price, cost_price, sku,
    category_id, stock_quantity, is_featured, is_on_sale,
    sale_percentage, material_type, weight_grams, care_instructions,
    color_options, images = []
  } = req.body;

  console.log('Creating product:', { name, price, category_id });
  ...
  try {
    ...
    const result = await client.query(
      `INSERT INTO products (
        ...
        category_id, stock_quantity, is_featured, is_on_sale, sale_percentage,
        material_type, weight_grams, care_instructions, color_options, is_active
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        await client.query(
          `INSERT INTO product_images (product_id, image_url, alt_text, is_primary, sort_order)
           VALUES ($1, $2, $3, $4, $5)`,
          [productId, imageUrl, img.alt_text || name, i === 0, i]
        );
...
const updateProduct = async (req, res) => {
  ...
  try {
    ...
    const updateFields = {
      ...
      sale_percentage: sale_percentage ? parseInt(sale_percentage) : null,
      material_type: material_type || null,
      weight_grams: weight_grams ? parseFloat(weight_grams) : null,
      ...
      if (images.length > 0) {
        for (let i = 0; i < images.length; i++) {
          ...
          await client.query(
            `INSERT INTO product_images (product_id, image_url, alt_text, is_primary, sort_order)
             VALUES ($1, $2, $3, $4, $5)`,
            [id, imageUrl, img.alt_text || name || 'Product', i === 0, i]
          );
        }
      }
    }
  ...
  try {
    const stats = await pool.query(`
      SELECT 
        ...
        (SELECT COUNT(*) FROM flash_sale_settings WHERE is_active = true AND start_time <= NOW() AND end_time >= NOW()) as active_flash_sales
    `);
...
const uploadImages = async (req, res) => {
  try {
    ...
    console.log('Upload complete. Returning:', uploadedImages.length, 'images');
  ...
  try {
    const result = await pool.query(
      `SELECT id, username, email, first_name, last_name, phone, 
              address, city, county, is_admin, created_at, updated_at
       FROM users 
...
const updateUserRole = async (req, res) => {
  ...
  try {
    ...
    // Don't allow modifying your own role
